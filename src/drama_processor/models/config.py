...
class FeishuWatcherConfig(BaseModel):
    ...
    max_dates_per_cycle: int = Field(default=1, description="单次轮询最多同时触发的日期任务数")
    settle_seconds: int = Field(default=120, description="同一日期任务在无新剧时继续等待的秒数")
    settle_rounds: int = Field(default=2, description="连续空轮次数，超过后认为该日期暂时无新任务")
    ...
    date_whitelist: Optional[List[str]] = Field(default=None, description="仅监听的日期列表")
    date_blacklist: Optional[List[str]] = Field(default=None, description="需要忽略的日期列表")
    status_filter: Optional[str] = Field(default=None, description="覆盖默认的状态过滤值")
...
class ProcessingConfig(BaseModel):
    ...
    bottom_opacity: float = Field(default=0.85, ge=0.0, le=1.0, description="Bottom text opacity (0.0-1.0)")
    ...
    hook_text_color: str = Field(default="#FFE600", description="Hook text color (bold, no border for performance)")
    hook_letter_spacing: int = Field(default=10, description="Hook text letter spacing in pixels (5-15 recommended)")

...
# 创建达人专属的飞书监控启动脚本
# 自动使用 PowerShell 以支持 Unicode/emoji
$feishuBatContent = @"
@echo off
...
if %errorlevel% equ 0 (
    echo Starting Feishu Watcher in PowerShell...
    echo.
    start "Feishu Watcher - ${Name}" powershell.exe -NoExit -Command "& {Set-Location '%~dp0drama-processor'; if (Test-Path 'venv\Scripts\activate.ps1') {Write-Host '========================================' -ForegroundColor Cyan; Write-Host '  Drama Processor - Feishu Watcher' -ForegroundColor Cyan; Write-Host '  User: ${Name}' -ForegroundColor Cyan; Write-Host '========================================' -ForegroundColor Cyan; Write-Host ''; .\venv\Scripts\activate.ps1; python -m drama_processor --config configs\default.yaml feishu watch} else {Write-Host '[ERROR] Virtual environment not found!' -ForegroundColor Red; Write-Host 'Please run: run-install.bat' -ForegroundColor Yellow; pause}}"
    exit /b 0
)
